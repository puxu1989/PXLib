...
namespace PXLib.Maths
{
    ...
    public class DESArithmetic
    {
        ...
        private void Copy(byte[] src, int srcIndex, ref byte[] dst, int dstIndex, int count)
        {
        ...
        /// <param name="dstIndex"></param>
        /// <param name="count"></param>
        private void Copy(char[] src, int srcIndex, ref char[] dst, int dstIndex, int count)
        {
            if (src == null || srcIndex < 0 ||
                dst == null || dstIndex < 0 || count < 0)
            {
        ...
        private static byte[] S_BOXSINGLE = {
			...
			15, 12, 9,7,3,10,5,0,15,12,8,2,4,
        ...
        private void generateKey(byte[] Key)
        {
            ...
            this.Copy(Key, 0, ref newKey, 0, Math.Min(Key.Length, newKey.Length));
        ...
        private void transform(byte[] outb, int outIndex, byte[] inb, int inIndex, byte[] Table, int len)
        {
            ...
            this.Copy(TRANSFORM_TMP, 0, ref outb, outIndex, len);
        }
        ...
        private void rotateL(byte[] inb, int startIndex, int len, int loop)
        {
            ...
            this.Copy(inb, startIndex, ref ROTATEL_TMP, 0, loop);
            ...
            this.Copy(ROTATEL_TMP, 0, ref inb, startIndex + len - loop, loop);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        private void desRun(byte[] outb, int outIndex, byte[] inb, int inIndex, byte type)
        {
            ...
            else
            {
                for (int i = 15; i >= 0; i--)
                {
                    this.Copy(DES_RUN_M, 0, ref DES_RUN_TMP, 0, 32);
                    Byte[] tmpSubKey = (Byte[])subKey[i];
                    function(DES_RUN_M, 0, tmpSubKey);
                    xor(DES_RUN_M, 0, DES_RUN_M, 32, 32);
                    this.Copy(DES_RUN_TMP, 0, ref DES_RUN_M, 32, 32);
                }
            }
        ...
        private char[] transCode(byte[] data)
        {
            ...
            for (int i = 0, k = 0; i < len; i++)
            {
                ...
                else
                {
                    buffer[k++] = '%';
                    index++;
                    buffer[k++] = (char)HEXCODE[this.MoveByte(tempChar, 4) & 15]; // >>>
                    index++;
            ...
            this.Copy(buffer, 0, ref result, 0, index);
            return result;
        }
        ...
        private byte[] deTransCode(char[] data)
        {
            ...
            this.Copy(buffer, 0, ref result, 0, k);
            return result;
        }
