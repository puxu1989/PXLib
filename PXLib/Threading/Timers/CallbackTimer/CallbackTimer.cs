...
namespace PXLib.Threading.Timers.CallbackTimer
{
    ...
    public class CallbackTimer<T> : BaseCycleEngine, ICallbackTimer<T>
    {
        ...
        protected override bool DoDetect()
        {
            ...
            lock (this.locker)
            {
                taskList = CollectionHelper.CopyAllToList<CallbackTask<T>>(this.dicTask.Values);
            }
        ...
        #region AddCallback
        public int AddCallback(int spanInSecs, Action<T> _callback, T _callbackPara)
        {
            lock (this.locker)
            {
                ++this.idCount;
        ...
        public int RemoveCallbackAndAddNew(int taskIDToRemoved, int spanInSecs, Action<T> _newCallback, T _newCallbackPara)
        {
